...
function Customize() {
  ...
  try {
    const res = await axios.post(`${serverurl}/api/assistant/create`, {
      assistantName,
      assistantImage,
 ...
 const handleBack = () => {
    if (!userData?.assistantImage || !userData?.assistantName) {
      alert("Please finish customizing your assistant before going back.");
    } else {
      navigate("/"); 
    }
  };
  ...
  return (
    <div className="w-full h-[100vh] bg-gradient-to-t from-[black] to-[#030353] flex justify-center items-center flex-col p-[20px]">
       <MdKeyboardBackspace className='absolute top-[25px] left-[25px] text-white cursor-pointer w-[25px] h-[25px]' onClick={handleBack}/>
...
<div className="w-full max-w-[900px] flex justify-center items-center flex-wrap gap-[15px]">
        ...
        <Card image={image7}/>
         ...
         hover:4 hover:border-white flex items-center justify-center ${selectedImage==="input"?
          "border-4 border-white shadow-2xl shadow-blue-950":null}`} onClick={() => {
          inputImage.current.click()
          setSelectedImage("input")
        }}>
          {!frontendImage && <BiSolidImageAdd className='text-white w-[25px] h-[25px]'/>}
